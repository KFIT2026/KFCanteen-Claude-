...
/* ── footer (shown on every page, fixed height so it never shifts between pages) ──
   ...
   footer's full width), so the gap between them stays a fixed, modest size
   instead of growing into empty space on wide screens. Each company's own
    ...
    <div style={{maxWidth:1100,margin:"0 auto",width:"100%",display:"flex",flexDirection:"column",alignItems:"center",gap:10,padding:"0 1.5rem"}}>
      <div style={{display:"flex",justifyContent:"center",alignItems:"start",gap:0,flexWrap:"wrap"}}>
        ...
        <div style={{height:48,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <img src="/logos/koufu-mis.webp" alt="Kou Fu MIS" style={{height:38,width:"auto"}} />
        </div>
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
          <div style={{height:48,display:"flex",alignItems:"center"}}>
  ...
  const renderTab = () => {
    ...
    if(activeTab==="mgorders") {
      ...
      {paymentModal&&(
          ...
          {/* order count + sales summary -- both mirror filteredOrders exactly
              (same date/search/plant filters as the table below), so they
              read as "totals for what you're currently looking at": the
              selected date, or every record if "Show all records" is checked. */}
          {(()=>{
            const cashOrders = filteredOrders.filter(o=>o.paymentType==="Cash");
            const creditOrders = filteredOrders.filter(o=>o.paymentType==="Credit");
            const unpaidOrders = filteredOrders.filter(o=>!o.paymentType);
            ...
            return <div style={{display:"flex",gap:24,marginBottom:16,flexWrap:"wrap"}}>
              <div>
                ...
                <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                  ...
                  <div style={{background:"#fff",borderRadius:10,border:"1px solid #E5E7EB",padding:"10px 18px",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
                    <span style={{fontSize:20,fontWeight:800,color:"#9CA3AF"}}>{unpaidOrders.length}</span>
    ...
    if(activeTab==="expenses") {
      ...
      // order counts for the same period + search-filtered employee set,
      // to go alongside the peso totals below (rows only tracks pesos per
      // employee, not counts broken down by payment type).
      const rowIds = new Set(rows.map(r=>r.id));
      const periodRowOrders = periodOrders.filter(o=>rowIds.has(o.userId));
      const cashOrderCount = periodRowOrders.filter(o=>o.paymentType==="Cash").length;
      const creditOrderCount = periodRowOrders.filter(o=>o.paymentType==="Credit").length;
      const pendingOrderCount = periodRowOrders.filter(o=>!o.paymentType).length;
      ...
      return (
        <div>
          ...
          {/* order count + sales summary -- same "Order Records" / "Sales
              Records" layout as Manage Orders / Overall History, for
              consistency. Both respect the current period (month or custom
              range) and the employee search filter. */}
